...
async function runProductionFixTests() {
  ...
  try {
    ...
    const updateCount = await prisma.$executeRawUnsafe(`
      ...
      WHERE scheduled_date_key IS NULL OR scheduled_date_key = '' OR scheduled_date_key !~ '^[0-9]{4}-[0-9]{2}-[0-9]{2}$'
    `);
    ...
    // =========================================================================
    // TEST 3: Legacy Task with scheduledDateKey = '' on Today
    // =========================================================================
    console.log('Testing Test 3: Legacy task with scheduledDateKey = empty string...');
    const legacyTaskId = randomUUID();
    const nowIso = new Date().toISOString();

    await prisma.$executeRawUnsafe(`
      INSERT INTO tasks (id, user_id, title, topic, status, task_type, scheduled_date, scheduled_date_key, created_at, updated_at)
      VALUES ('${legacyTaskId}', '${testUserId}', 'Introduction to Priority Queues using Binary Heaps', 'Heaps', 'pending', 'new', '${nowIso}', '', NOW(), NOW())
    `);

    const hitlistLegacy = await taskRepository.getTodaysTasks(testUserId, tz);
    assert(hitlistLegacy.some((t) => t.id === legacyTaskId), 'Legacy empty-key task on today IS included in getTodaysTasks');

    // Test backlogCron on empty-key task for today
    await runBacklogCron();
    const legacyCheckAfterCron = await prisma.task.findUnique({ where: { id: legacyTaskId } });
    assert(legacyCheckAfterCron?.status === 'pending' && !legacyCheckAfterCron?.isBacklog, 'Backlog cron did NOT mark legacy task for today as backlog');
    console.log('✅ Test 3 passed');

    // Clean up legacy task
    await prisma.task.delete({ where: { id: legacyTaskId } });

    // =========================================================================
    // TEST 4: POTD Visibility Across Timezone Mismatch
    // =========================================================================
    console.log('Testing Test 4: POTD visibility with timezone mismatch parameter...');
    ...
    // =========================================================================
    // TEST 5: Dashboard Service Integration without Active Plan
    // =========================================================================
    console.log('Testing Test 5: Dashboard service integration (no active plan)...');
    ...
    // =========================================================================
    // TEST 6: Dismissed POTD Non-Resurrection
    // =========================================================================
    console.log('Testing Test 6: Dismissed POTD handling...');
    ...
    // =========================================================================
    // TEST 7: POTD Streak UTC Semantics
    // =========================================================================
    console.log('Testing Test 7: POTD Streak calculation...');
    ...
    // =========================================================================
    // TEST 8: Archived Plan Task Isolation
    // =========================================================================
    console.log('Testing Test 8: Archived plan task isolation...');
